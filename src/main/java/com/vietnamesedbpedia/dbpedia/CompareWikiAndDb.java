...
/**
 * Created by vietanknb on 31/5/2017.
 */
...
//    private static String MAIN_FOLDER = "template_statistic" + File.separator + "infobox-Thong_tin_don_vi_hanh_chinh";
    ...
    private static String MAIN_FOLDER_INPUT = "data";
    public static String MAIN_FOLDER_OUTPUT = "result";

    public static void main(String args[]) {
        String nameInfobox = "Thông_tin_khu_dân_cư";

         compareUsingEnMapping(nameInfobox);
        //compare using Algorithm
        File file = new File(MAIN_FOLDER_INPUT +File.separator+nameInfobox, "wiki");
        CompareWikiAndDb compareWikiAndDb = new CompareWikiAndDb();
        MergeOnePage mergeOnePage = new MergeOnePage();
        MergeResult mergeResult = new MergeResult();
        if (file.isDirectory()) {
            String names[] = file.list();
            for (String a : names) {
                compareWikiAndDb.compare(a.replace(".txt", ""), Constants.ENGLISH, nameInfobox);
                compareWikiAndDb.compare(a.replace(".txt", ""), Constants.GERMANY, nameInfobox);
                compareWikiAndDb.compare(a.replace(".txt", ""), Constants.NETHERLAND, nameInfobox);
                mergeOnePage.merge(nameInfobox, a.replace(".txt",""));
            }
        }

        mergeResult.mergeAll(nameInfobox);
    }

    private static void compareUsingEnMapping(String namefile) {
        CompareWikiAndDb compareWikiAndDb = new CompareWikiAndDb();
//

        File file1 = new File(MAIN_FOLDER_OUTPUT, namefile);
        ...
        try {
            ...
            File f = new File(MAIN_FOLDER_INPUT+File.separator+namefile, namefile + ".xls");
...
//                }
            ...
            compareWikiAndDb.compareWithMappingFile(attributes, namefile + "_mapping_en", nameInfobox, mapVietAtrrToEnAttr, namefile);
    ...
    public void compareWithMappingFile(ArrayList<Attribute> attributes, String fileResult, String nameInfobox, HashMap<String, String> mapVietAtrrToEnAttr, String nameFile) {
...
//        }
        ...
        writeToFileExcel(name, resultMapping, nameFile);
    }

    private void writeToFileExcel(String name, HashMap<Attribute, String> resultMapping, String nameFile) {
        try {
//            File file = new File(MAIN_FOLDER_OUTPUT, nameFile);
//            if(!file.isDirectory()){
//               file.mkdir();
//            }
            ...
            WorkbookSettings ws = new WorkbookSettings();
            ws.setEncoding("Cp1252");
            WritableWorkbook workbook = Workbook.createWorkbook(new File(MAIN_FOLDER_OUTPUT+File.separator+nameFile,name), ws);
            ...
            for (Attribute a : attributes) {
                ...
                if(resultMapping.get(a)==null||resultMapping.get(a).equals("")){
                    sheet.addCell(new Number(3, index, 0));
                    sheet.addCell(new Number(4, index, 0.0, numberFormat));
                }else{
                    sheet.addCell(new Number(3, index, Double.valueOf(a.getCount())));
                    sheet.addCell(new Number(4, index, 1.0, numberFormat));
                }
    ...
    public void compare(String name, String language, String nameInfobox) {
        Logger logger = Logger.getLogger(Test.class);
        logger.debug("Start");
        ...
        try {
            ...
            //lay ten wiki page tuong ung trong ngon ngu en, de, nl
...
//            // Ho_Chi_Minh
            ...
            //lay ra tat ca cac properties cua bai viet trong ngon ngu en, de, nl
            if(language.equals(Constants.ENGLISH)){
            ...
            //lay ra tap cac attribute tuong ung trong infobox
            attributes = getDataAttribute(MAIN_FOLDER_INPUT + File.separator + nameInfobox+File.separator+"wiki"+File.separator + name + ".txt");
...
//                                    logger.error("textTokens: " +textTokens);
                                    if (!tooManyTokens) {
                                        ngramsLengthForString = min(tokenizedValue, textTokens.toArray(new String[textTokens.size()]));
...
//                                    results.get(template).add(label, k);
                                    ...
                                    if (newTokens.size() > 0 && newTokens.size() <= tokenLimit) {
                                        ngramsLengthForString = min(tokenizedValue, newTokens.toArray(new String[newTokens.size()]));
                                        for (int j = ngramsLengthForString; j > 0; j--) {
                                            sk = new StringKernel(lambda, j);
...
//                        String b_translate_bing_vi = com.vietnamesedbpedia.dbpedia.TranslateDemo.translateQuickBingApi("en", "vi", p_name).toLowerCase();
        ...
        try {
            ...
            File file = new File(MAIN_FOLDER_OUTPUT+File.separator+nameInfobox, "wiki");
            if(!file.isDirectory()){
                file.mkdir();
            }
            File file2 = new File(file.getPath(), "detail");
            if(!file2.isDirectory()){
                file2.mkdir();
            }
            WritableWorkbook workbook = Workbook.createWorkbook(new File(MAIN_FOLDER_OUTPUT + File.separator + nameInfobox + File.separator + "wiki" + File.separator +"detail",  name + ".xls"));
            WritableSheet sheet = workbook.createSheet("Sheet 1", 0);
    ...
    private int min(String[] wikiToken, String[] dbpediaToken){
        int min = 2;

        int a = wikiToken.length;
        int b = dbpediaToken.length;
        if(a>2 && b>2){
            if(a>b){
                min = b;
            }else{
