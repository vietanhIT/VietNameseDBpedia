        ...
        for (Long number : intValues) {
            if (floatValue != 0) {
                if ((Math.abs(floatValue - number) / floatValue) < precision) {
                    logger.debug("Added " + p.getName() + "---" + a.getName());
...
//                    results.get(template).add(label);
                    ...
                    if ((Math.abs(floatValue - number) / floatValue) < precision) {
                        logger.debug("Added (after pow) " + p.getName() + "---" + a.getName());
...
//        System.out.println(range);
        ...
        if (file.isDirectory()) {
            ...
            for (String a : names) {
                test.compare(a.replace(".txt", ""),Constants.ENGLISH);
            }
        }
    }
...
//                }
            ...
            HashMap<String,String> h1 = Search.getViInterLanguageLink("Bản_mẫu:"+namefile);
            String nameInfobox ="";
            if(h1.size()>0){
    ...
    public void compare(String name, String language) {
        ...
        try {
            ...
            if(language.equals(Constants.ENGLISH)){
                h4 = Search.getViInterLanguageLink(name);
            }else if(language.equals(Constants.GERMANY)){
                h4 = Search.getViInterLanguageLink_de(name);
            }else{
                h4 = Search.getViInterLanguageLink_nl(name);
            }
...
//                                    logger.debug("textTokens: " +textTokens);
                                    if (!tooManyTokens) {
                                        for (int j = ngramsLengthForString; j > 0; j--) {
                                            ...
                                            if (k > 0) {
                                                logger.debug("Added " + p.getName() + " " + a.getName() + " " + k);
...
//                                    results.get(template).add(label, k);
                                    ...
                                    for (String link : links) {
                                        logger.debug("Searching translation for " + link);
                                        HashMap<String, String> h3 = new HashMap<>();
                                        if(language.equals(Constants.ENGLISH)){
                                            h3 = Search.getViInterLanguageLink(link);
                                        }else if(language.equals(Constants.GERMANY)){
                                            h3 = Search.getViInterLanguageLink_de(link);
                                        }else{
                                            h3 = Search.getViInterLanguageLink_nl(link);
                                        }
                                        String stringTranslation = h3.get(link);
                                        if (stringTranslation == null || stringTranslation.equals("")) {
                                            logger.debug("Value translation not found");
                                            continue;
                                        }
                                        logger.debug("Translation found: " + stringTranslation);
...
//                                    logger.trace("Text cleaned");
                        ...
                        HashMap<String, String> h3;
                        if(language.equals(Constants.ENGLISH)){
                            h3 = Search.getInterLanguageLink(p.getValue());
                        }else if(language.equals(Constants.GERMANY)){
                            h3 = Search.getInterLanguageLink_de(p.getValue());
                        }else{
                            h3 = Search.getInterLanguageLink_nl(p.getValue());
                        }
                        ...
                        if (valueTranslation != null && links.contains(valueTranslation)) {
                            logger.debug("Added " + p.getName() + " --- " + a.getName() + " ---- " + links.toString() + " --- " + valueTranslation);
                            double t = 0.0;
...
//                                        results.get(globalLabel).add(label, k);
        ...
        for (String key : keys) {
            ...
            logger.debug("Result: " + key + " -- " + h1.toString());
...
//                ArrayList<String> list_a_translate_gg_en = TranslateDemo.translateQuickGoogle("vi", "en", a);
//                String a_translate_bing_en = TranslateDemo.translateQuickBingApi("vi", "en", a).toLowerCase();
                ...
                for(String n: k2s){
                    ...
                    try {
//                        ArrayList<String> list_b_translate_gg_vi = TranslateDemo.translateQuickGoogle("en", "vi", b);
//                        String b_translate_bing_vi = TranslateDemo.translateQuickBingApi("en", "vi", b).toLowerCase();
                        ...
                        logger.debug("Translate: " + a + "( gg: " + list_a_translate_gg_en.toString() + ") ( bing: " + a_translate_bing_en + " )" + " " + b);
                        logger.debug("Translate: " + b + "( gg: " + list_b_translate_gg_vi.toString() + ") ( bing: " + b_translate_bing_vi + " )" + " " + a);
                        logger.debug(a_translate_bing_en.equals(b) + " ----- " + list_a_translate_gg_en.contains(b));
                        logger.debug(b_translate_bing_vi.equals(a) + " ----- " + list_b_translate_gg_vi.contains(a));
                        if (a_translate_bing_en.equals(b) || list_a_translate_gg_en.contains(b)||b_translate_bing_vi.equals(a) || list_b_translate_gg_vi.contains(a_remove_mark)) {
                            ...
                            //logger.debug(resultAftermax.get(a).size());
        ...
        try{
            SearchDBpediaAllDump searchDBpediaAllDump = new SearchDBpediaAllDump("ALL_DB_DUMP");
            ArrayList<String> propertieNames = searchDBpediaAllDump.getAllPropertyNameFromDBpediaByTitleArticle(nameTran);
            logger.debug("Translate a name");
            for(Attribute a: attributes){
                String a_name = modelString(a.getName()).trim();
                String a_name_remove_mark = TranslateDemo.removeDiacriticalMarks(a_name);
//                ArrayList<String> list_a_translate_gg_en = TranslateDemo.translateQuickGoogle("vi", "en", a_name);
//                String a_translate_bing_en = TranslateDemo.translateQuickBingApi("vi", "en", a_name).toLowerCase();
                ...
                for(String p: propertieNames){
                    ...
                    try {
//                        ArrayList<String> list_b_translate_gg_vi = TranslateDemo.translateQuickGoogle("en", "vi", p_name);
//                        String b_translate_bing_vi = TranslateDemo.translateQuickBingApi("en", "vi", p_name).toLowerCase();
                        ...
                        logger.debug("Translate: " + a_name + "( gg: " + list_a_translate_gg_en.toString() + ") ( bing: " + a_translate_bing_en + " )" + " " + p_name);
                        logger.debug("Translate: " + p_name + "( gg: " + list_b_translate_gg_vi.toString() + ") ( bing: " + b_translate_bing_vi + " )" + " " + a_name);
                        logger.debug(a_translate_bing_en.equals(p_name) + " ----- " + list_a_translate_gg_en.contains(p_name));
                        logger.debug(b_translate_bing_vi.equals(a_name) + " ----- " + list_b_translate_gg_vi.contains(a_name_remove_mark));
